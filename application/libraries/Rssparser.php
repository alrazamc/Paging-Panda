	...
	function parse()
	{
		...
		if (!isset($rawFeed))
		{
			if(ini_get('allow_url_fopen')){
				...
				$rawFeed = curl_browser_request($this->feed_uri);
		}
		try
		{
		...
		if ($xml->channel)
		{
			...
			foreach ($xml->channel->item as $item)
			{
				...
				if($item->children($namespaces['media'])->thumbnail)
					$data['image_url'] = (string) $item->children($namespaces['media'])->thumbnail->attributes()->url;
				else if($item->children($namespaces['media'])->content)
					$data['image_url'] = (string) $item->children($namespaces['media'])->content->attributes()->url;
		...
		else
		{
			...
			foreach ($xml->entry as $item)
			{
				...
				$data['link'] = (string)$item->link[2]['href'];
		...
		if ($this->write_cache_flag)
		{
			...
			{
				echo "RSSParser error: Couldn't write the to cache";
