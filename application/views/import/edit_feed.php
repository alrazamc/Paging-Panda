...
<script type="text/javascript">
  ...
  angapp.controller('pageController', function($scope, $http, $sce, $filter, $timeout){
    ...
    $scope.feed_id = '<?php echo $feed->rss_feed_id ?>';
    $scope.feed_url = '<?php echo $feed->rss_feed_url ?>';
    $scope.feed_title = '<?php echo $feed->rss_feed_name ?>';
    $scope.to_pending = '<?php echo $feed->to_pending ?>';
    $scope.feed_category_id = '<?php echo $feed->category_id ?>';
    $scope.feed_use_once = '<?php echo $feed->use_once ?>';
    $scope.feed_pages = <?php echo json_encode($feed->accounts) ?>;

    $timeout(function(){
        for(var i=0; i < $scope.pages.length; i++)
            if($scope.feed_pages.indexOf( parseInt($scope.pages[i].account_id) ) !== -1 )
                $scope.pages[i].selected = true;
            else
                $scope.pages[i].selected = false;
            if($scope.feed_use_once == 1)
                $scope.selected_category = $scope.categories[1];
            else if($scope.feed_category_id == 0)
                $scope.selected_category = $scope.categories[0];
            else{
                for(var i=0; i<$scope.categories.length; i++)
                    if($scope.categories[i].category_id == $scope.feed_category_id)
    ...
    $scope.send_feed = function()
    {
        ...
        $http.post(api_base_url + 'import/update/' + $scope.feed_id, {feed:feed}).then(function(response){
...
<div class="container my-5" ng-app="app" ng-controller="pageController">
    <div class="row">
        <div class="col-sm-8 mb-3">
            <h4 class="page-title">Edit RSS Feed</h4>
        </div>
